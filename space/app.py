...
# Find wherever src/ actually lives relative to this file, rather than
# assuming a fixed depth -- this exact file is also deployed standalone at
# the root of a Hugging Face Space (alongside its own copy of src/configs),
# not just nested under the main repo's app/ folder, and the two layouts
# put src/ at a different ancestor level.
for _candidate_root in (Path(__file__).resolve().parent, Path(__file__).resolve().parents[1] if len(Path(__file__).resolve().parents) > 1 else None):
    if _candidate_root is not None and (_candidate_root / "src").is_dir():
...
CACHED_ANSWERS_PATH = Path(__file__).parent / "cached_answers.json"


def log_interaction(question: str, outcome: GuardrailOutcome, response_text: str, latency_s: float,
                     served_from_cache: bool = False) -> None:
...
def load_cached_answers() -> dict[str, str]:
    """Pre-captured, GENUINE answers from an actual trained-model run,
    keyed by exact question text. Used so a small set of pre-agreed demo
    questions respond instantly and reliably regardless of how slow live
    generation is on whatever hardware the app happens to run on (e.g. a
    free, CPU-only Hugging Face Space) -- every value in this file must be
    real model output someone captured and pasted in, never invented text.
    Starts empty (file absent) until that capture happens; live generation
    is used for everything not in this file."""
    if not CACHED_ANSWERS_PATH.exists():
        return {}
    with open(CACHED_ANSWERS_PATH, encoding="utf-8") as f:
        return json.load(f)
...
def make_answer_fn(service: InferenceService, cached_answers: dict[str, str] | None = None):
    ...
    def answer_fn(question: str) -> str:
        ...
        if question in cached_answers:
            # Instant, reliable playback of a REAL captured answer -- see
            # load_cached_answers(). Still goes through G3 (disclaimer) via
            # apply_disclaimer(), same as every other path.
            from src.serve.guardrails import apply_disclaimer

            text = apply_disclaimer(cached_answers[question])
            outcome = GuardrailOutcome.ANSWERED
            ...
            try:
                ...
                # "no model configured" placeholder -- apply_disclaimer() is
                # the same helper guarded_response() uses for real answers,
                # so this path is not a way to accidentally skip FR6.
                from src.serve.guardrails import apply_disclaimer
        ...
        log_interaction(question, outcome, text, latency, served_from_cache=served_from_cache)
        return text
